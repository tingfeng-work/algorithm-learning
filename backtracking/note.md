...
扩展题目：357. 统计各位数字都不同的数字个数

### 概述

排列型回溯就是元素的顺序不同，视为不同的集合，例如 [1,2] 与 [2,1] 在计算答案时，算不同的答案。从枚举答案选哪个的角度来说，每次选择时，只要当前轮次没被选过，都可以继续选；而之前的子集型或组合型是之前轮次选过了，就不能选了。所以，在实现时，需要一个数组维护当前轮次选过的元素。

### 结合具体题目

46.全排列：满足条件的答案集合的长度等于数组的长度，所以当枚举到第 nums.length 个答案时，记录答案。同时维护一个布尔数组 flag，flag[i] 表示当前轮次中 nums[i] 元素已经使用过了，一共有 nums.length 轮，由于 path 的长度固定，所以可以通过初始化固定长度的 list，通过覆盖的方式实现恢复现场。

51.N 皇后：将复杂问题按照条件逐步拆分为子问题与已经解决的问题：用 queens 数组记录皇后位置，即 queens[i] 表示第 i 行 第 queens[i] 列有皇后，皇后处在不同行不同列，其实就是 queesns 数组的一个全排列问题。皇后不能处于同一斜线上，就是有限制条件的全排列问题。

### 心得体会

通常全排列的时间复杂度就等于叶子节点的个数，同时全排列需要一个布尔数组用来记录当前访问的元素在当前轮次中是否访问过，也就是每次访问需要从当前轮次没有加入答案的集合中选择。

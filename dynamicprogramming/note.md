...
变形题目：1039.多边形三角剖分的最低得分、375.猜数字大小Ⅱ

### 概述

之前的线性 DP 主要关注前缀/后缀的最优解，例如前 i 个元素的最长公共子序列，而区间 DP 状态的转移是在一个区间内，回溯中体现在传递的参数包括区间起点与结束点，`dp[i][j]` 数组的含义表示在区间 [i,j] 内的最优解。

### 结合具体题目

516.最长回文子序列：枚举区间的左右两个端点，如果两个端点对应的字符相同，则回文子序列长度+2，否则分类讨论跳过左端点，跳过右端点；i>j 时，区间为空，回文子序列长度为 0，i = j 时只有一个字母，符合要求，返回长度为 1. 不难推断出`dp[i][j]` 表示区间 [i,j] 内的最长回文子序列。在翻译为递推时，需要考虑循环顺序，由于 `dp[i][j]` 依赖于 `dp[i+1][·]` 所以必须倒序枚举，同时 `dp[i][j]` 依赖于 `dp[i][j-1]`，所以从小到大枚举

> 注意：
>
> * 区分最长回文子序列与字串的区别，子序列不一定是连续的，字串需要连续
>
> * 由于 dp 数组不会访问到 -1 所以数组长度不用 +1 

1039.多边形三角剖分的最低得分：这道题的难点在于怎么将其划分为一个与原问题相似的子问题。考虑无论怎么剖分，多边的一边一定在一个三角形中，所以固定一个边，枚举另一个顶点，将原图像分为左、三角形、右三个子图像中，当前划分的得分就是左边的多边形的得分+当前构成的三角形+右边多边形的得分。顶点的区间为[0,len-1] , 固定一个边，就是固定两个顶点，不妨假设两个端点为固定的边，问题转化为在 (i,j) 中找到一个顶点 k，使 i、j、k 构成的三角形得分+(i,k)为固定边的多边形最低得分+(k,j)为固定边的最低得分。

>多边形三角剖分抽象出来就是在区间 (i,j) 中找到一个分割点 k，使得区间划分 (i,k) 、k、(k,j) 为最优解

375.猜数字大小 Ⅱ：这道题类似多边形三角剖分，将 [1,n] 区间分为 [1,k-1], k, [k+1,n]，当前花费为 k，子问题求子区间 [1,k-1], [k+1,n] 的最小现金数，最终当前分割下的答案就是 res = max(dfs(1,k-1), dfs(k+1,n)) + k，取最大值是为了保证能够猜中。枚举每种分割情况取最小值，即为答案

### 心得体会

区间 DP 关注的是一个区间内的最优解，而一个区间内的最优解又可以通过划分多个小区间，来求小区间的最优解得到，最终就递归到边界条件。由于递推的起点的区间一半为空或一个元素，所以dp数组的遍历顺序通常是一正一反，具体谁正谁反要根据当前状态要依赖前一个状态还是后一个状态。

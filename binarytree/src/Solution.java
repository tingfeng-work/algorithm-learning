...
//        return Math.max(left, right) + 1;
    ...
    private void f(TreeNode root, int cnt) {
        if (root == null) return;
        cnt++;
...
//    }
    ...
    public int sumOfLeftLeaves(TreeNode root) {
         ...
         * 404. 左叶子之和
         * 给定二叉树的根节点 root ，返回所有左叶子之和。
         * 思路：当前根节点的左叶子之和 = 左子树的左叶子之和+右子树的左叶子之和 // 有点难
         * 自顶向下
         * */
//        dfs(root);
//        return ans;
        ...
        TreeNode left = root.left;
        if (left != null && left.left == null && left.right == null) sum = sum + left.val;
        return sum;
...
//        if (root == null) return;
//        num = num * 10 + root.val;
//        if (root.left == null && root.right == null) ans = ans + num;
//        dfs(root.left, num);
//        dfs(root.right, num);
//    }
...
//    }
    ...
    public List<List<Integer>> verticalTraversal(TreeNode root) {
         ...
         * 思路：深度优先遍历的同时，维护当前节点所处的坐标，
         * 遍历到当前节点时，根据坐标加入集合，首先集合按列排列，每个集合内，元素按行排列，行相同按值的大小排列
         * */
        Map<Integer, List<int[]>> groups = new TreeMap<>();
        dfs(root, 0, 0, groups);
        List<List<Integer>> ans = new ArrayList<>();
        for (List<int[]> value : groups.values()) {
            value.sort((a, b) -> {
                return a[0] == b[0] ? a[1] - b[1] : a[0] - b[0];
            });
            List<Integer> vals = new ArrayList<>(value.size());
            for (int[] ints : value) {
                vals.add(ints[1]);
            }
            ans.add(vals);
    ...
    private void dfs(TreeNode root, int row, int col, Map<Integer, List<int[]>> groups) {
        if (root == null) return;
...
//        int sum = node.val;
//        int left = dfs(node.left, map), right = dfs(node.right, map);
//        sum = sum + left + right;
//        map.merge(sum, 1, Integer::sum);
...
//        return dfs(root, Long.MIN_VALUE, Long.MAX_VALUE);
        ...
        // 中序遍历：得到的是有序序列，只需记录前一个节点值，判断是否严格小于即可
//        if (root==null) return true;
...
//        int val = node.val;
//        if (val > left[1] && val < right[0]) return new long[]{Math.min(left[0], val), Math.max(right[1], val)};
//        return new long[]{Long.MIN_VALUE, Long.MAX_VALUE};
//    }
...
//                dfs(root.left, minValue, val) &&
//                        dfs(root.right, val, maxValue);
//    }
    ...
    public TreeNode searchBST(TreeNode root, int val) {
        ...
        else if (x > val) return searchBST(root.left, val);
    ...
    public int rangeSumBST(TreeNode root, int low, int high) {
         ...
         * */
        if (root == null) return 0;
        int sum = 0, x = root.val;
        if (x < low) sum += rangeSumBST(root.right, low, high);
        else if (x > high) sum += rangeSumBST(root.left, low, high);
        else sum = sum + x + rangeSumBST(root.right, low, high) + rangeSumBST(root.left, low, high);
        return sum;
    }
    ...
    public List<List<Integer>> closestNodes(TreeNode root, List<Integer> queries) {
         ...
         * mini 是树中小于等于 queries[i] 的 最大值 。如果不存在这样的值，则使用 -1 代替。
         * maxi 是树中大于等于 queries[i] 的 最小值 。如果不存在这样的值，则使用 -1 代替。
         * 思路：树的节点树为 n，查询数为 m，如果每个查询数都去遍历一遍树，最终的时间复杂度是 O(mn),
         * m与n是一个数量级的，也就相当于 O(n^2)
         * 如果先中序遍历二叉搜索树，得到有序数组，再在有序数组上二分，最终复杂度就是O(nlogn)
         * */
        List<List<Integer>> ans = new ArrayList<>();
        List<Integer> list = new ArrayList<>();
        dfs(root, list);
        ...
        for (int query : queries) {
            int index = binarySearch(array, query); // 第一个大于等与query的数
            int min, max;
            if (index == array.length) {
                max = -1;
                min = array[index - 1];
            } else if (array[index] == query) {
                min = query;
                max = query;
            } else {
                max = array[index];
                min = index > 0 ? array[index - 1] : -1;
            }
            ans.add(List.of(min, max));
        }
        return ans;
    ...
    private int binarySearch(int[] array, int query) {
        int left = -1, right = array.length;

        while (left + 1 < right) {
            int mid = (left + right) >>> 1;
            if (array[mid] >= query) right = mid;
            else left = mid;
        }
        return right;
    }
...
//        for (Integer i : cnt.keySet()) {
//            Integer count = cnt.get(i);
//            if (count == max) ans.add(i);
//        }
//        return ans.stream().mapToInt(Integer::intValue).toArray();
        // 思路：利用中序遍历的有序性，众数只会连续出现
        if (root == null) return null;
...
//            ans.clear();
//        }
//        if (cnt == maxCnt) ans.add(root.val);
//        dfs(root.right, ans);
//    }

//

    //    private void dfs(TreeNode root, Map<Integer, Integer> cnt) {
//        if (root == null) return;
...
//    }
    public int kthSmallest(TreeNode root, int k) {
        ...
        dfs(root, list);
        return list.stream().mapToInt(Integer::intValue).toArray()[k - 1];
    }

    private void dfs(TreeNode node, Set<Integer> list) {
        if (node == null) return;
        dfs(node.left, list);
        list.add(node.val);
        dfs(node.right, list);
    ...
    public int maxSumBST(TreeNode root) {
         ...
         * 思路：
         * XX采用中序遍历，维护pre为前一个节点的值，sum为当前遍历的键值和，maxSum为最大键值和
         * 如果当前节点值的大于pre，则将其加入 sum，如果当前sum大于 maxSum 则更新
         * 如果当前节点值小于等于pre，则将 sum 置位当前节点值
         * 采用后序遍历，利用后序遍历先判断当前树是否二叉搜索树
         * */
        dfs(root);
        return maxSum;
    ...
    /*
     * dfs[0] : 0 表示该树不是是二叉搜索树；1 表示该树是二叉搜索树
     * dfs[1]: 表示最小值
     * dfs[2]: 表示最大值
     * dfs[3]: 表示键值和
     * */
    private int[] dfs(TreeNode node) {
        ...
        int[] left = dfs(node.left);
        if (left[0] == 0) return new int[]{0, 0, 0, 0};
        int[] right = dfs(node.right);
        int val = node.val;
        if (right[0] == 1 && val > left[2] && val < right[1]) {
            // 该树是二叉搜索树
            int sum = left[3] + right[3] + val;
            if (sum > maxSum) maxSum = sum;
            return new int[]{1, Math.min(val, left[1]), Math.max(val, right[2]), sum};
        } else {
            //该树不是二叉搜索树
...
//            map.put(inorder[i], i);
//        }
//        return dfs(0, n, 0, preorder, map);
//    }
//    private TreeNode dfs(int preL, int preR, int inL, int[] preorder, Map<Integer, Integer> map) {
//        if (preL == preR) return null;
//        int leftSize = map.get(preorder[preL]) - inL;
//        TreeNode left = dfs(preL + 1, preL + 1 + leftSize, inL, preorder, map);
//        TreeNode right = dfs(preL + 1 + leftSize, preR, inL + leftSize + 1, preorder, map);
//        return new TreeNode(preorder[preL], left, right);
//    }
    public TreeNode buildTree(int[] inorder, int[] postorder) {
//        int n = inorder.length;
...
//        node.left = buildTree(in1, post1);
//        node.right = buildTree(in2, post2);
//        return node;
        int n = inorder.length;
        Map<Integer, Integer> map = new HashMap<>(n);
        for (int i = 0; i < inorder.length; i++) {
        ...
        return dfs(0, n, 0, n, postorder, map);
    }

    private TreeNode dfs(int inL, int inR, int poL, int poR, int[] postorder, Map<Integer, Integer> map) {
        if (inL == inR) return null;
        int leftSize = map.get(postorder[poR - 1]) - inL;
        TreeNode left = dfs(inL, inL + leftSize, poL, poL + leftSize, postorder, map);
        TreeNode right = dfs(inL + leftSize + 1, inR, poL + leftSize, poR - 1, postorder, map);
...
//        int leftSize = findIndex(postorder, preorder[1]) + 1;
//        int[] pre1 = Arrays.copyOfRange(preorder, 1, leftSize + 1);
//        int[] pre2 = Arrays.copyOfRange(preorder, 1 + leftSize, n);
//        int[] pos1 = Arrays.copyOfRange(postorder, 0, leftSize);
//        int[] pos2 = Arrays.copyOfRange(postorder, leftSize, n - 1);
...
//        return new TreeNode(preorder[0], left, right);
        ...
        return dfs(0, n, 0, n, preorder, index);
    }

    private TreeNode dfs(int preL, int preR, int posL, int posR, int[] preorder, int[] index) {
        if (preL == preR) return null;
        if (preL + 1 == preR) return new TreeNode(preorder[preL]);
        int leftSize = index[preorder[preL + 1]] + 1 - posL;
        TreeNode left = dfs(preL + 1, preL + 1 + leftSize, posL, posL + leftSize, preorder, index);
        TreeNode right = dfs(preL + 1 + leftSize, preR, posL + leftSize, posR - 1, preorder, index);
        return new TreeNode(preorder[preL], left, right);
    }
    ...
    public List<TreeNode> delNodes(TreeNode root, int[] to_delete) {
         ...
         * 返回森林中的每棵树。你可以按任意顺序组织答案。
         * */
        Set<Integer> list = new HashSet<>(to_delete.length);
        for (int i : to_delete) {
            list.add(i);
        }
        List<TreeNode> ans = new ArrayList<>();
        if (dfs(root, list, ans) != null) ans.add(root);

        return ans;
    }

    private TreeNode dfs(TreeNode node, Set<Integer> list, List<TreeNode> ans) {
        if (node == null) return null;
        node.left = dfs(node.left, list, ans);
        node.right = dfs(node.right, list, ans);
        int val = node.val;
...
//         * 236. 二叉树的最近公共祖先
//         * 给定一个二叉树, 找到该树中两个指定节点的最近公共祖先。
//         * 思路：先序遍历二叉树，如果当前节点是 p 或 q 或为空，直接返回当前节点
//         * 否则递归的遍历左子树，查找 q、p 是否在左子树中，遍历右子树，查找 p、q是否在右子树中
//         * */
...
//    }
    ...
    public TreeNode lowestCommonAncestor(TreeNode root, TreeNode p, TreeNode q) {
         ...
         * 思路：当 val>p 且 val > q 时，q、p 在右子树，不用遍历左子树
         * 同理 val<q 且 val < p 时，q、p 在左子树，不用遍历右子树
         * 否则直接返回当前节点
         * */
        int val = root.val;

...
//    }
    public List<String> letterCombinations(String digits) {
         ...
         * 17. 电话号码的字母组合
         * 给定一个仅包含数字 2-9 的字符串，
         * 返回所有它能表示的字母组合。答案可以按任意顺序返回。
         * 思路：首先创建一个hash表用来映射数字到字母的关系
         * 最后字母的组合的长度 = digits.lenth
         * 枚举第 i 个字母选哪个
         * 子问题变为大于 i+1 的字母选哪个
         * */
        List<String> map = List.of("", "", "abc", "def", "ghi", "jkl", "mno", "pqrs", "tuv", "wxyz");
        ...
        dfs(0, path, ans, digits, map);
        return ans;
    }

    private void dfs(int i, char[] path, List<String> ans, String digits, List<String> map) {
        if (i == digits.length()) {
            ans.add(new String(path));
            return;
        }
    ...
    public List<List<Integer>> subsets(int[] nums) {
         ...
         * 思路：枚举第 i 个元素选不选
         * 子问题：在下标大于 i 的元素中选择元素来构造子集
         * 下一个子问题： 在下标大于 i+1 的元素中来构造子集
         * */
//        List<List<Integer>> ans = new ArrayList<>();
//        List<Integer> path = new ArrayList<>();
//        dfs(0, nums, path, ans);
...
//    }
    ...
    public List<List<String>> partition(String s) {
         ...
         * 考虑第 i 个字符与 i+1 之间是否分割
         * 当前操作：选则第 i 与 第 i+1 之间是否分割
         * 子问题：从剩下的为分割的字符中中构造回文串
         * 下一个操作：枚举第 i + 1 与 i+2 之间是否分割
         * */
        List<List<String>> ans = new ArrayList<>();
        List<String> path = new ArrayList<>();
        dfs(0, path, ans, s);
    ...
    //    // i 表示第 i个逗号选不选
//    private void dfs(int i, int start, List<String> path, List<List<String>> ans, String s) {
//        if (i == s.length()) {
//            ans.add(new ArrayList<>(path));
//            return;
//        }
...
//
    ...
    public List<List<Integer>> combine(int n, int k) {
         ...
         * 77. 组合
         * 给定两个整数 n 和 k，返回范围 [1, n] 中所有可能的 k 个数的组合。
         * 思路：倒序枚举答案选哪个，当 path 的长度等于 k 时，记录答案
         * */
        List<List<Integer>> ans = new ArrayList<>();
        List<Integer> path = new ArrayList<>();
//        dfs(ans, path, n, k); 选哪个
        dfs(n, k, path, ans);
        return ans;
    }
...
//    }
    ...
    // 选不选
    private void dfs(int i, int t, int k, List<Integer> path, List<List<Integer>> ans) {
        int d = k - path.size();
...
//    }
    ...
    private void dfs(int i, int open, int n, char[] path, List<String> ans) {
        if (i == n * 2) {
            ans.add(new String(path));
            return;
        }
    ...
    public List<List<Integer>> permute(int[] nums) {
         ...
         * 思路：全排列就是在组合的条件下，可以选择之前选择过的元素，只是不能选当前选了的元素
         * 枚举答案选哪个 i 表示第 i 个答案，set 表示还没有选择的元素
         * */
        List<List<Integer>> ans = new ArrayList<>();
        int n = nums.length;
        List<Integer> path = Arrays.asList(new Integer[n]);
    ...
    //    枚举答案选哪个 i 表示path中第i个元素，set 表示还没有选择的元素
    private void dfs(int i, int[] nums, boolean[] onPath, List<Integer> path, List<List<Integer>> ans) {
        if (i == nums.length) {
            ans.add(new ArrayList<>(path));
            return;
    ...
    public List<List<String>> solveNQueens(int n) {
         ...
         * */
        List<List<String>> ans = new ArrayList<>();
        int[] col = new int[n]; // 表示第 i 行 第 col[i] 列放放皇后
        boolean[] onPath = new boolean[n];
        dfs(0, col, onPath, ans);
        return ans;
    }

    private void dfs(int row, int[] col, boolean[] onPath, List<List<String>> ans) {
        if (row == col.length) {
            StringBuilder sb = new StringBuilder();
            List<String> path = new ArrayList<>();
    ...
    private boolean valid(int row, int i, int[] col) {
        for (int r = 0; r < row; r++) {
            int c = col[r];
            if (r + c == row + i || r - c == row - i)
                return false;
        }
        return true;
